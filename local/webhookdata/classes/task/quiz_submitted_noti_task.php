...
class quiz_submitted_noti_task extends \core\task\adhoc_task
{
    public function get_name()
    {
        return get_string("quizsubmit", "local_webhookdata");
    }
    ...
    public function execute()
    {
        ...
        $user = $DB->get_record('user', ['id' => $eventdata->other->submitterid]);
        $course = $DB->get_record('course', ['id' => $eventdata->courseid], 'fullname');
        $url = local_webhookdata_get_course_callback($eventdata->courseid);
        $quiz = $DB->get_record('quiz', ['id' => $eventdata->other->quizid]);

        // Append related data to event data.
        $eventdata->url = $url;
        $eventdata->student = $user->firstname . ' ' . $user->lastname;
        $eventdata->course = $course->fullname;
        $eventdata->subject = $quiz->name;
        if($quiz->timeclose != 0) {
            $eventdata->timeclose = date('Y-m-d H:i:s',$quiz->timeclose);
        }
        $eventdata->event = 'MOODLE_QUIZ_SUBMIT';
